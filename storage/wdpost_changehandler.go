...
func (ch *changeHandler) currentTSDL() (*types.TipSet, *dline.Info) {
	return ch.submitHdlr.currentTSDL()
}
...
func (p *proveHandler) processHeadChange(ctx context.Context, newTS *types.TipSet, di *dline.Info) {
	...
	if p.di != nil && newTS.Height() >= p.di.Close {
		p.abort()
		p.di = nil
	}
	...
	if newTS.Height() > di.Challenge {
		p.di = di
		p.abort = p.api.startGeneratePoST(ctx, newTS, di, func(posts []miner.SubmitWindowedPoStParams, err error) {
			p.postResults <- &postResult{di: di, posts: posts, err: err}
		})
	}
}
...
type submitHandler struct {
	...
	currentLk  sync.RWMutex
	currentCtx context.Context
	currentTS  *types.TipSet
	currentDL  *dline.Info
...
func (s *submitHandler) processHeadChangeForPW(ctx context.Context, revert *types.TipSet, advance *types.TipSet, pw *postWindow) {
	...
	if pw.submitState == SubmitStateSubmitting && (revertedToPrevDL || expired) {
		...
		// immediately submit again without the state getting clobbered
		// when the old submit completes
		pw = &postWindow{
			di:          pw.di,
			ts:          advance,
			submitState: SubmitStateStart,
	...
	if pw.submitState == SubmitStateComplete && revertedToPrevDL {
...
func (s *submitHandler) processSubmitResult(res *submitResult) {
	if res.err != nil {
		...
		log.Warnf("Aborted window post %s (Deadline: %+v)", SubmitStateSubmitting, res.pw.di)
...
func (s *submitHandler) currentTSDL() (*types.TipSet, *dline.Info) {
	s.currentLk.RLock()
	defer s.currentLk.RUnlock()

	return s.currentTS, s.currentDL
}
